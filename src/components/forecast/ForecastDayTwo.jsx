...
function ForecastDayTwo() {
  ...
  const dayTwo = new Date(daily[2].dt * 1000).toLocaleDateString('en-GB', { weekday: 'short', day: 'numeric' });

  const weatherInfo = daily[2].weather[0].main;
  const weatherDescription = daily[2].weather[0].description;
    ...
    <Grid item xs={2.5} className="forecast-container">
      <Stack direction="column" spacing={0.5} justifyContent="center" alignItems="center">
        ...
        <Typography variant="p" className="forecast-maxTemp">
          {Math.round(daily[2].temp.max)}
          °
        </Typography>
        <Typography variant="p" className="forecast-minTemp">
          {Math.round(daily[2].temp.min)}
          °
        </Typography>
        ...
        <Stack direction="row" spacing={1} alignItems="center" justifyContent="center">
          ...
          <Typography variant="p" className="forecast-precipitation">
            {daily[2].rain ? Math.round(daily[2].rain) : '0'}
            %
          </Typography>
        </Stack>
      </Stack>
