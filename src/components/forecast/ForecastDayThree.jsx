...
function ForecastDayTwo() {
  ...
  const dayThree = new Date(daily[3].dt * 1000).toLocaleDateString('en-GB', { weekday: 'short', day: 'numeric' });

  const weatherInfo = daily[3].weather[0].main;
  const weatherDescription = daily[3].weather[0].description;
    ...
    <Grid item xs={2.5} className="forecast-container">
      <Stack direction="column" spacing={0.5} justifyContent="center" alignItems="center">
        ...
        <Typography variant="p" className="forecast-maxTemp">
          {Math.round(daily[3].temp.max)}
          °
        </Typography>
        <Typography variant="p" className="forecast-minTemp">
          {Math.round(daily[3].temp.min)}
          °
        </Typography>
